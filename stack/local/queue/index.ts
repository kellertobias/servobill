...
async function enqueueJob(job: {
  ...
  await runAsync(`INSERT INTO jobs (id, type, source, resources, data) VALUES (?, ?, ?, ?, ?)`, [
    job.eventId,
    job.type,
    job.source,
...
async function dequeueJob() {
  const row = await getAsync<any>(`SELECT * FROM jobs ORDER BY created_at ASC LIMIT 1`);
  if (!row) {
    return null;
  }
...
app.all('/', async (req, res) => {
  if (req.headers['x-amz-target'] !== 'AWSEvents.PutEvents') {
    console.log("Received Request that is not 'AWSEvents.PutEvents'", req.headers['x-amz-target']);
  ...
  for (const entry of entries) {
    ...
    console.log(chalk.bgBlue('[EventBus] IN '), DetailType, chalk.grey(JSON.stringify(rest)));
...
const handleNextEvent = async () => {
  ...
  if (!handlerImport) {
    console.log(chalk.bgBlue('[EventBus] Error '), chalk.red(`No handler for event type ${type}`));
    return;
  }
    ...
    setInterval(
      async () => {
        try {
          ...
          console.error(chalk.bgBlue('[EventBus] CRON'), 'Failed to enqueue cron event:', error);
        }
      },
      5 * 60 * 1000
