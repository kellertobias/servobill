...
export function handleFile(filepath: string, commonPrefix: string) {
  ...
  if (layersRaw) {
    ...
    layers = layersRaw.split(',').map((layer) => layer.trim().replaceAll(/["']/g, ''));
  }

  // Check if the file exports a variable named handlerName
  const handlerName = file.match(/export const handlerName = ["'](\w+)["']/)?.[1] || 'handler';
  ...
  const importsReflectMetadata = file.match(/import ["']reflect-metadata["'];/)?.[0];

  if (!importsReflectMetadata) {
    // eslint-disable-next-line no-console
    console.log(chalk.red.bold(`Missing import of reflect-metadata in ${relativePath}`));
    if (process.argv.includes('--watch')) {
      return;
    } else {
...
const printEndpointLine = (endpoint: ApiEndpoint | EventEndpoint, change: string) => {
  // eslint-disable-next-line no-console
  console.log(
    `${change}${
