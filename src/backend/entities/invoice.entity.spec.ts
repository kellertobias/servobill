...
import { InvoiceSubmissionEntity, InvoiceSubmissionType } from './invoice-submission.entity';
...
describe('InvoiceEntity', () => {
  ...
  it('should update status, number, and dates on first email submission', async () => {
    ...
    const activity = await invoice.addSubmission(submission, 'Alice', getSettings);
  ...
  it('should throw if trying to cancel from non-SENT status', () => {
    ...
    expect(() => invoice.updateStatus(InvoiceStatus.CANCELLED, 'Bob')).toThrowError();
  });
  ...
  it('should throw in updatePdf if contentHash is missing', () => {
    invoice.contentHash = undefined;
    expect(() => invoice.updatePdf({ bucket: 'b', region: 'r', key: 'k' })).toThrowError();
  });
  ...
  it('should set status to PAID_PARTIALLY on partial payment', () => {
    ...
    invoice.addPayment({ paidCents: 1000, paidAt: new Date(), paidVia: 'cash' }, 'Bob');
  ...
  it('should set status to PAID on overpayment', () => {
    invoice.totalCents = 1000;
    invoice.addPayment({ paidCents: 1500, paidAt: new Date(), paidVia: 'cash' }, 'Bob');
    expect(invoice.status).toBe(InvoiceStatus.PAID);
  });
