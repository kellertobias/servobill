...
import { DomainEntity, type DomainEntityKeys, DomainEvent } from './abstract.entity';
import type { CustomerEntity } from './customer.entity';
import type { ExpenseEntity } from './expense.entity';
import { InvoiceActivityEntity, InvoiceActivityType } from './invoice-activity.entity';
import type { InvoiceItemEntity } from './invoice-item.entity';
import { type InvoiceSubmissionEntity, InvoiceSubmissionType } from './invoice-submission.entity';
...
export class InvoiceEntity extends DomainEntity {
  ...
  public updateDates({
    ...
    if (this.status !== InvoiceStatus.DRAFT && this.status !== InvoiceStatus.SENT) {
  ...
  public updateTexts({ subject, footerText }: { subject?: string; footerText?: string }): void {
    if (subject) {
  ...
  public updateItems(items: InvoiceItemEntity[]): void {
    ...
    this.items = items;
    this.updatedAt = new Date();
    const subtotalCents = items.reduce((total, item) => total + item.getTotalCents(), 0);
    const totalTax = items.reduce((total, item) => total + item.getTotalTaxCents(), 0);
  ...
  public async addSubmission(
    ...
    if (scheduledSendJob) {
      ...
      const activity = new InvoiceActivityEntity({
        ...
        notes: scheduledSendJob
          ? `Scheduled to be sent at ${dayjs(scheduledSendJob?.runAfter * 1000).format(
              'DD.MM.YYYY HH:mm'
            )} (UTC)`
          : undefined,
    ...
    if (this.status === InvoiceStatus.DRAFT) {
      ...
      if (this.type === InvoiceType.INVOICE) {
        this.invoiceNumber = this.invoiceNumber ?? (await settings.invoiceNumbers.getNextNumber());
      } else {
        this.offerNumber = this.offerNumber ?? (await settings.offerNumbers.getNextNumber());
      }
    }
    ...
    if (this.type === InvoiceType.INVOICE) {
      ...
      this.dueAt =
        this.dueAt || dayjs(this.invoicedAt).add(settings.defaultInvoiceDueDays, 'days').toDate();
    } else {
      this.offeredAt = this.offeredAt || new Date();
      this.dueAt = this.dueAt || dayjs().add(settings.offerValidityDays, 'days').toDate();
    }
  ...
  public async cancelSubmission(deleteJob: (jobId: string) => Promise<void>) {
    ...
    const activity = this.activity.find((act) => act.ref === this.scheduledSendJobId);
    if (activity) {
  ...
  public addPayment(props: { paidCents: number; paidAt: Date; paidVia: string }, user: string) {
  ...
  public updateStatus(newStatus: InvoiceStatus, user: string): InvoiceActivityEntity {
