...
describe('EventBusService', () => {
  ...
  beforeEach(async () => {
    ...
    mockSend = ((await import('@aws-sdk/client-eventbridge')) as unknown as EventBridgeMockModule)
      .mocks.mockSend;
    mockLoggerInfo = ((await import('./logger.service')) as unknown as LoggerMockModule).mocks
      .mockLoggerInfo;
    const otel = (await import('@opentelemetry/api')) as unknown as OtelMockModule;
  ...
  it('should send an event and return the EventId', async () => {
    ...
    mockStartActiveSpan.mockImplementation((_name: string, cb: (span: FakeSpan) => unknown) =>
      cb(fakeSpan)
    );
    ...
    expect(mockLoggerInfo).toHaveBeenCalledWith('Sending event', expect.any(Object));
    expect(mockLoggerInfo).toHaveBeenCalledWith('Event Sent', expect.any(Object));
