...
export class LLMService {
   ...
   */
  private async sendAnthropicRequest(request: LLMRequest): Promise<LLMResponse> {
    if (!this.config.apiKey) {
  ...
  private async sendLocalRequest(request: LLMRequest): Promise<LLMResponse> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Local LLM API request failed: ${response.status} ${response.statusText}`);
    }
