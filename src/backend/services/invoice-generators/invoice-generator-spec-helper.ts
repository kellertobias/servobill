...
const getResult = (result: { content: Buffer; filename: string }[]): string => {
  ...
  const xml = result.find((attachment) => attachment.filename.endsWith('.xml'))?.content.toString();
...
export const runGenerator = async (
  ...
  const invoice = new InvoiceEntity({
    ...
    createdAt: new Date(),
    updatedAt: new Date(),
    invoicedAt: dayjs().startOf('day').add(16, 'hours').add(43, 'minutes').toDate(),
    dueAt: new Date(Date.now() + 1000 * 60 * 60 * 24 * 30),
    items: data.items,
    totalCents: data.items.reduce((acc, item) => acc + item.priceCents, 0),
    totalTax: data.items.reduce((acc, item) => acc + item.priceCents * item.taxPercentage, 0),
  ...
  invoice.totalCents = data.items.reduce((acc, item) => acc + item.priceCents, 0);
