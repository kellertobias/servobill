...
import type { CompanyDataSetting, VatStatus } from '@/backend/entities/settings.entity';
...
function getMonetarySummation(
  ...
  const taxBasisTotalAmount = (totals.lineNetAmount - totals.allowanceNetAmount).toFixed(2);

  const grandTotalAmount = (Number(taxBasisTotalAmount) + Number(taxTotalAmount)).toFixed(2);

  const paidAmount = invoice.paidCents ? Number(invoice.paidCents / 100).toFixed(2) : '0.00';
