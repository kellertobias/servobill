...
function mapInvoiceLineToUbl(
  ...
  return {
    ...
    'cbc:LineExtensionAmount': ((item.priceCents * item.quantity) / 100).toFixed(2),
...
function mapInvoiceToUbl(
  ...
  const allExempt =
    ...
    vatStatus === VatStatus.VAT_DISABLED_OTHER
      ? true
      : regularItems.length > 0 && regularItems.every((item) => item.taxPercentage === 0);
  if (allExempt) {
    taxGroups = [{ rate: 0, code: 'E', items: regularItems }];
  } else {
    const groups: Record<string, { rate: number; code: string; items: InvoiceItemEntity[] }> = {};
  ...
  const totalAllowances = Object.values(allowanceSumsByKey).reduce((sum, v) => sum + v, 0);
  ...
  const buyerReference = buyer.customerNumber || invoice.invoiceNumber || invoice.id;
  ...
  return {
    'cbc:CustomizationID': 'urn:cen.eu:en16931:2017#compliant#urn:fdc:gov.xrechnung.de:2017',
    'cbc:ProfileID': 'urn:fdc:peppol.eu:2017:poacc:billing:01:1.0',
    'cbc:ID': invoice.invoiceNumber || invoice.id,
    'cbc:IssueDate': (invoice.invoicedAt || invoice.createdAt || new Date())
      .toISOString()
      .slice(0, 10),
    'cbc:DueDate': invoice.dueAt ? invoice.dueAt.toISOString().slice(0, 10) : undefined,
    ...
    'cac:LegalMonetaryTotal': {
      ...
      ...(totalAllowances > 0
        ? {
            ...
            'cbc:AllowanceTotalAmount@currencyID': currency as InvoiceCurrencyCode,
          }
        : {}),
    },
    ...
    ...(allowanceCharges.length > 0 ? { 'cac:AllowanceCharge': allowanceCharges } : {}),
  };
}
