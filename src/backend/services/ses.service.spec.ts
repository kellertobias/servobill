...
describe('SESService', () => {
  ...
  beforeEach(() => {
    ...
    sendMailMock = vi.fn((opts, cb) => cb(null, { accepted: [opts.to], ...opts }));
    createTransportMock = vi.fn(() => ({ sendMail: sendMailMock }));
    (nodemailer.createTransport as unknown as typeof createTransportMock) = createTransportMock;
  ...
  it('should send email via SMTP with attachments', async () => {
    const config = makeSMTPConfig();
    const service = new SESService(config);
    const attachments = [{ filename: 'file.txt', content: Buffer.from('test') }];
  ...
  it('should reject if sendMail returns error', async () => {
    ...
    sendMailMock.mockImplementationOnce(
      (_opts: SendMailOptions, cb: (err: Error | null, info?: unknown) => void) =>
        cb(new Error('fail'))
    );
    const config = makeSMTPConfig();
    const service = new SESService(config);
