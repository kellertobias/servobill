...
describe('FileStorageServiceLocal', () => {
  ...
  beforeEach(() => {
    ...
    app = App.forRoot({
      modules: [{ token: CONFIG_SERVICE, value: config }, FileStorageServiceLocal],
    });
...
describe('FileStorageServiceS3', () => {
  ...
  it('should get a file (getFile)', async () => {
    ...
    expect(result).toEqual(Buffer.concat(chunks as unknown as readonly (Uint8Array & Buffer)[]));
  });

  it('should throw if file not found (getFile)', async () => {
    asMock(s3.getObject).mockResolvedValue(undefined);
    await expect(service.getFile('key.txt')).rejects.toThrow('File not found in S3');
  });
  ...
  it('should get upload url (getUploadUrl)', async () => {
    ...
    const directService = new FileStorageServiceS3(makeConfig(FileStorageType.S3), s3);
