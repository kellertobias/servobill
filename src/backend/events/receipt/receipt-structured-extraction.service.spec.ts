  ...
  const testCases = [
    ...
    },
    {
      ...
      expect: (result: ExtractedInvoiceStructure, xml: string) => {
        ...
        const total = result.lineItems.reduce((sum: number, li: any) => sum + li.totalCents, 0);
        expect(total, xml).toBe(2 * 500 * 1.19 + 1 * 2000 * 1.07);

        expect(result.totals?.netCents, xml).toBe(2 * 500 + 1 * 2000);
        expect(result.totals?.taxCents, xml).toBe(2 * 500 * 0.19 + 1 * 2000 * 0.07);
        expect(result.totals?.grossCents, xml).toBe(2 * 500 * 1.19 + 1 * 2000 * 1.07);
      },
    },
    {
    ...
    },
    {
      ...
      expect: (result: ExtractedInvoiceStructure, xml: string) => {
        ...
        expect(result.lineItems, xml).toHaveLength(1);
        expect(result.lineItems[0].name, xml).toBe('Avocado, Roast Beef, Discount Coupon');
        expect(result.totals?.taxCents, xml).toBe(2 * 500 * 0.19 + 1 * 2000 * 0.07 - 300 * 0.19);
        expect(result.totals?.grossCents, xml).toBe(2 * 500 * 1.19 + 1 * 2000 * 1.07 - 300 * 1.19);
