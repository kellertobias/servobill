...
export function generateExpensesSummaryHtml(
  ...
  const totalAmount = expenses.reduce((sum, expense) => sum + expense.expendedCents, 0);
  const totalTax = expenses.reduce((sum, expense) => sum + (expense.taxCents || 0), 0);
  ...
  const attachmentsList = attachments.map((attachment) => `<li>${attachment.name}</li>`).join('');
