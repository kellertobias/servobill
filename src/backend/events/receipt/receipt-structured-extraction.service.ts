...
import { EXPENSE_REPOSITORY, type ExpenseRepository } from '@/backend/repositories';
import { Logger } from '@/backend/services/logger.service';
import { Inject, Service } from '@/common/di';
import { extractEmbeddedXmlWithPdfLib } from './extract-pdf-xml';
import type { ReceiptExtractorService, ReceiptResult } from './receipt-interface';
import type {
 ...
 */
@Service()
export class ReceiptStructuredExtractionService implements ReceiptExtractorService {
  ...
  private detectInvoiceFormat(parsedXml: InvoiceXml): InvoiceFormat {
    ...
    if (rootKeys.some((k) => k.toLowerCase().includes('crossindustryinvoice'))) {
    ...
    if (rootKeys.some((k) => k.toLowerCase().includes('invoice'))) {
      ...
      const invoiceKey = rootKeys.find((k) => k.toLowerCase().includes('invoice'))!;
  ...
  private async categorizeWithLLM(
    ...
    const expense = new ExpenseEntity({
      ...
      expendedCents: structure.lineItems.reduce((acc, item) => acc + item.totalCents, 0),
      ...
      taxCents: structure.lineItems.reduce((acc, item) => acc + item.taxCents, 0),
    });
