...
async function createPdfWithEmbeddedXml(xmlContent: string): Promise<Buffer> {
  ...
  // Embed the XML file
  await pdfDoc.attach(new Uint8Array(Buffer.from(xmlContent, 'utf8')), 'invoice.xml', {
...
<invoice>
    ...
    await pdfDoc.attach(new Uint8Array(Buffer.from(xmlContent1, 'utf8')), 'invoice1.xml', {
      mimeType: 'application/xml',
      description: 'First E-Invoice XML',
    });

    await pdfDoc.attach(new Uint8Array(Buffer.from(xmlContent2, 'utf8')), 'invoice2.xml', {
      mimeType: 'application/xml',
