...
export class ZugferdExtractorStrategy extends ReceiptStructuredStrategy {
   ...
   */
  private getInvoiceRoot(parsedXml: Record<string, unknown>): Record<string, unknown> | undefined {
    const keys = ['rsm:CrossIndustryInvoice', 'CrossIndustryInvoice', 'Invoice'];
  ...
  private extractSellerName(invoice: Record<string, unknown>): string {
    ...
    const seller = this.getFirstObject(agreement, ['ram:SellerTradeParty', 'SellerTradeParty']);
    return this.getFirstStr(seller?.['ram:Name']) || this.getFirstStr(seller?.Name) || '';
  }
  ...
  private extractInvoiceDate(invoice: Record<string, unknown>): Date {
    const header = this.getFirstObject(invoice, ['rsm:ExchangedDocument', 'ExchangedDocument']);

    // Try to get date from nested structure
    const dateTimeObj = this.getFirstObject(header, ['ram:IssueDateTime', 'IssueDateTime']);
    ...
    if (dateTimeObj) {
      ...
      dateStr = this.getFirstStr(dateTimeStringObj) || this.getFirstStr(dateTimeObj);
    }
  ...
  private extractInvoiceNumber(invoice: Record<string, unknown>): string {
    const header = this.getFirstObject(invoice, ['rsm:ExchangedDocument', 'ExchangedDocument']);
    const invoiceNumber =
      this.getFirstStr(header?.['ram:ID']) || this.getFirstStr(header?.ID) || '';
  ...
  private extractSubject(invoice: Record<string, unknown>): string {
    const header = this.getFirstObject(invoice, ['rsm:ExchangedDocument', 'ExchangedDocument']);
    const noteObj = this.getFirstObject(header, ['ram:IncludedNote', 'IncludedNote']);
    return this.getFirstStr(noteObj?.['ram:Content']) || this.getFirstStr(noteObj?.Content) || '';
  }
}
