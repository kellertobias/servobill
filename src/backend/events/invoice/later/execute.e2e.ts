...
interface TestInvoice {
  ...
  >;
  addSubmission: (submission: unknown, userName: string, settingsLoader: unknown) => Promise<void>;
  createAndLinkExpensesForInvoice: (cb: (data: unknown) => Promise<unknown>) => Promise<void>;
}
...
function createMockInvoice({
  ...
  return {
    ...
    createAndLinkExpensesForInvoice: async (cb: (data: unknown) => Promise<unknown>) => {
      await cb({});
    },
  };
}
...
describe('HandlerExecution (send later invoice event) E2E', () => {
  ...
  it('should process the event, remove scheduled submission, add EMAIL submission, and create expenses', async () => {
    ...
    const event = new InvoiceSendLaterEvent(makeEventFields(invoiceId, 'test-user'));
    ...
    expect(invoice.submissions.some((s) => s.type === InvoiceSubmissionType.EMAIL)).toBe(true);
    // Assert: expense was created
    expect(expenseRepository.created.length).toBe(1);
  });

  it('should throw if invoice not found', async () => {
    const event = new InvoiceSendLaterEvent(makeEventFields('not-exist', 'test-user'));
  ...
  it('should throw if no scheduled job found', async () => {
    ...
    invoiceRepository.invoices.push(invoice);
    const event = new InvoiceSendLaterEvent(makeEventFields('inv-2', 'test-user'));
    await expect(handler.execute(event)).rejects.toThrow('No scheduled job found for this invoice');
  });
  ...
  it('should throw if original scheduled submission not found', async () => {
    ...
    invoiceRepository.invoices.push(invoice);
    const event = new InvoiceSendLaterEvent(makeEventFields('inv-3', 'test-user'));
    await expect(handler.execute(event)).rejects.toThrow('Original scheduled submission not found');
  });
  ...
  it('should throw if original scheduled submission is already cancelled', async () => {
    ...
    invoiceRepository.invoices.push(invoice);
    const event = new InvoiceSendLaterEvent(makeEventFields('inv-4', 'test-user'));
