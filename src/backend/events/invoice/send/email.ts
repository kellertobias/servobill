...
export class InvoiceEmailSender {
  ...
  public async getAttachments(invoice: InvoiceEntity) {
    ...
    const flaggedAttachmentActivities = invoice.activity.filter(
      (a) => a.type === InvoiceActivityType.ATTACHMENT && a.attachToEmail && a.attachmentId
    );
  ...
  private async getSubject(invoice: InvoiceEntity, companyData: CompanyDataSetting) {
    const subjects = {
    ...
    const subjectTemplate = subjects[invoice.type === InvoiceType.INVOICE ? 'invoice' : 'offer'];
  ...
  private async getEmail(invoice: InvoiceEntity, companyData: CompanyDataSetting) {
    const { html } = await this.cqrsBus.execute(
      new GenerateInvoiceHtmlCommand({
        logoUrl: companyData.emailCompanyLogo,
