...
import { INVOICE_EMAIL_SENDER, type InvoiceEmailSender } from './email';
import type { InvoiceSendEvent } from './event';
...
export class HandlerExecution {
  ...
  async execute(event: InvoiceSendEvent) {
    ...
    const outputFormat = invoiceSettings.invoiceOutputFormat as InvoiceOutputFormat;
    ...
    const extraAttachments = await this.invoiceEmailSender.getAttachments(invoice);
    ...
    await this.invoiceEmailSender.sendEmail(event.id, invoice, companyData, attachments);
  ...
  private async getGenerator(outputFormat: InvoiceOutputFormat, invoiceType: InvoiceType) {
