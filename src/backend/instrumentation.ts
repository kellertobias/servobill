...
export function Span(spanName: string, attributes?: Record<string, AttributeValue>) {
...
const withEventBridgeInstrumentation = async <E extends EventBridgeEvent<string, unknown>, R>(
  spanInfo: {
    name: string;
    attributes?: Record<string, AttributeValue>;
  ...
  return handleSpanExecution({ span, onThrow: spanInfo.onThrow }, handler, [evt, ctx, cb]);
};

const withApiGatewayInstrumentation = async <E extends APIGatewayProxyEventV2, R>(
  spanInfo: {
    name: string;
    attributes?: Record<string, AttributeValue>;
    onThrow?: (error: Error) => Awaited<R> | null;
  ...
  return tracer.startActiveSpan(spanInfo.name, async (span) => {
    ...
    return await handleSpanExecution({ span, onThrow: spanInfo.onThrow }, handler, [evt, ctx, cb]);
  });
};
...
const handleSpanExecution = async <E, R>(
  ...
  try {
    ...
    if (onThrow) {
      const errorResult = onThrow(error instanceof Error ? error : new Error(String(error)));
      if (errorResult === null) {
        throw error;
      } else {
        answer = errorResult;
...
const withWrappedInstrumentation = <E, R>(
  ...
  return tracer.startActiveSpan(spanInfo.name, (span) => {
    ...
    return handleSpanExecution({ span, onThrow: spanInfo.onThrow }, handler, args);
  });
};
