...
@CqrsHandler(CreatePdfCommand)
export class CreatePdfHandler implements ICqrsHandler<CreatePdfCommand> {
	private logger = new Logger(CreatePdfHandler.name);
	constructor(
	...
	@Span('CreatePdfHandler.generatePdf')
	...
	async execute(command: CreatePdfCommand['request']) {
		const { html } = command;
		...
		if (!command.key && !command.invoice) {
			throw new Error(
				'Either key or invoice must be provided to generate filename',
			);
		}
