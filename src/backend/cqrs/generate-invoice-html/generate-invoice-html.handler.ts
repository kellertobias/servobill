...
import { CqrsHandler, type ICqrsHandler } from '@/backend/services/cqrs.service';
...
export class GenerateInvoiceHtmlHandler implements ICqrsHandler<GenerateInvoiceHtmlCommand> {
  private buildData(command: GenerateInvoiceHtmlCommand['request']) {
    return {
      name: command.invoice.customer.contactName || command.invoice.customer.name,
      company: {
        ...command.company,
      ...
      subtotal: `${centsToPrice(
        command.invoice.items.reduce((acc, item) => acc + item.priceCents * item.quantity, 0)
      )}€`,
      tax: `${centsToPrice(
        command.invoice.items.reduce(
          (acc, item) => acc + item.priceCents * item.quantity * (item.taxPercentage / 100),
          0
        )
      )}€`,
      total: `${centsToPrice(
        command.invoice.items.reduce(
          (acc, item) => acc + item.priceCents * item.quantity * (1 + item.taxPercentage / 100),
          0
        )
      )}€`,
      paid: command.invoice.paidCents ? `${centsToPrice(command.invoice.paidCents)}€` : false,
  ...
  private registerHelpers(command: GenerateInvoiceHtmlCommand['request']) {
    handlebars.registerHelper('date', (date, format) => {
      ...
      return dayjs(date).format(typeof format === 'string' && format ? format : 'DD.MM.YYYY');
    });
    handlebars.registerHelper(
      'nl2br',
      (text: string) => new handlebars.SafeString(`${text || ''}`.replaceAll('\n', '<br />'))
    );
