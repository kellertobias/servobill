...
const INSECURE_COOKIES =
  process.env.INSECURE_COOKIES === 'true' || process.env.NODE_ENV === 'development';
...
export class Session {
  ...
  constructor(props: Omit<Session, 'renewalId' | 'sessionId'> & Partial<Session>) {
...
const makeTokenCookieInternal = (content: Session | null, type: 'SESSION' | 'REFRESH'): string => {
  if (!JWT_SECRET) {
  ...
  return cookie.serialize(type === 'SESSION' ? SESSION_COOKIE_NAME : REFRESH_COOKIE_NAME, token, {
    httpOnly: true,
...
export const makeTokenCookie = {
  session: (content: Session | null): string => makeTokenCookieInternal(content, 'SESSION'),
  refresh: (content: Session | null): string => makeTokenCookieInternal(content, 'REFRESH'),
};
...
const getRawTokenFromCookies = (
  ...
  const cookieName = type === 'SESSION' ? SESSION_COOKIE_NAME : REFRESH_COOKIE_NAME;
...
const getRawTokenFromEvent = (
  ...
  const cookies = cookie.parse(evt.cookies?.join?.(';') || evt.headers.cookie || '');

  return (type === 'SESSION' ? headerToken : undefined) || getRawTokenFromCookies(cookies, type);
};

const extractTokenInternal = (
  evt: APIGatewayProxyEventV2 | { headers?: undefined; cookies: Record<string, string> },
  type: 'SESSION' | 'REFRESH'
): JwtToken | null => {
  if (!JWT_SECRET) {
    throw new Error('JWT_SECRET not set');
...
export const extractToken = {
  session: (evt: APIGatewayProxyEventV2 | { cookies: Record<string, string> }) =>
    extractTokenInternal(evt, 'SESSION'),
  refresh: (evt: APIGatewayProxyEventV2 | { cookies: Record<string, string> }) =>
