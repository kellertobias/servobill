...
import { extractToken, type JwtToken, makeTokenCookie, Session } from '../session';
...
export class AuthenticationService {
    ...
    const cookies =
      session === SessionOp.UNCHANGED || session === undefined
        ...
        : session === SessionOp.END
          ...
          : [makeTokenCookie.session(session), makeTokenCookie.refresh(session)];
  ...
  public async startSession(
    ...
    const sessionEntity = await this.sessionRepository.createSession(
      new SessionEntity({
        userId: user.userId,
        renewalId,
        expiresAt: new Date(Date.now() + (options?.expireIn || DEFAULT_SESSION_EXPIRATION)),
      })
    );
  ...
  public async renewSession(
    ...
    const sessionEntity = await this.sessionRepository.getSession(data.sessionId);
