...
describe('InventoryTypeResolver (integration)', () => {
	...
	beforeAll(async () => {
		...
		app = testEnv.app;
		inventoryTypeRepo = app.get<InventoryTypeRepository>(
			INVENTORY_TYPE_REPOSITORY,
		);
		inventoryItemRepo = app.get<InventoryItemRepository>(
			INVENTORY_ITEM_REPOSITORY,
		);
		locationRepo = app.get<InventoryLocationRepository>(
    ...
    `;
		...
		const names: string[] = (
			res.data.inventoryTypes as Array<{ name: string }>
		).map((t) => t.name);
    ...
    `;
		...
		expect(errorMsg).toMatch(
			/Parent inventory type with id nonexistent not found/,
		);
	});
