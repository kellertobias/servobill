...
export class InventoryResolver {
  ...
  async inventoryItems(
    ...
    try {
      ...
      const mappedItems = await Promise.all(items.map((item) => this.mapToGraphQL(item)));
      return mappedItems;
    } catch (error) {
  ...
  async createInventoryItem(
    ...
    try {
      ...
      if (data.locationId) {
        const location = await this.inventoryLocationRepository.getById(data.locationId);
        if (!location) {
          throw new Error(`Inventory location with id ${data.locationId} not found`);
        }
      }
      ...
      item.updateProperties(data.properties?.map((prop) => [prop.key, prop.value]) || []);
  ...
  async updateInventoryItem(
    ...
    try {
      ...
      if (data.locationId !== undefined) {
        ...
        const oldLocation = await this.inventoryLocationRepository.getById(existingItem.locationId);
      ...
      if (data.properties !== undefined) {
        existingItem.updateProperties(data.properties.map(({ key, value }) => [key, value]));
      }
  ...
  @Authorized()
  @Mutation(() => Boolean)
  async deleteInventoryItem(@Arg('id', () => String) id: string): Promise<boolean> {
    try {
      const existingItem = await this.inventoryItemRepository.getById(id);
      if (!existingItem) {
        return false;
   ...
   */
  @Authorized()
  public async mapToGraphQL(entity: InventoryItemEntity): Promise<InventoryItem> {
    return {
      id: entity.id,
      name: entity.name,
  ...
  async location(@Root() item: InventoryItem): Promise<InventoryLocation | undefined> {
    if (!item || !item.id || !item.locationId) {
      return undefined;
    }
    const location = await this.inventoryLocationRepository.getById(item.locationId);
    if (!location) {
      return undefined;
    }
