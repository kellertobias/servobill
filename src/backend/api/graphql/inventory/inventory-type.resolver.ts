...
export class InventoryTypeResolver {
  ...
  async inventoryTypes(
    ...
    try {
      ...
      let queryWhere: Partial<InventoryTypeWhereInput> | undefined; // Use Partial for flexible filter
      if (where) {
        if (where.rootOnly) {
  ...
  async createInventoryType(
    ...
    try {
      ...
      if (data.parent) {
        const parentType = await this.inventoryTypeRepository.getById(data.parent);
        if (!parentType) {
          throw new Error(`Parent inventory type with id ${data.parent} not found`);
        }
      }
  ...
  @Authorized()
  @Mutation(() => Boolean)
  async deleteInventoryType(@Arg('id', () => String) id: string): Promise<boolean> {
    try {
      const existingType = await this.inventoryTypeRepository.getById(id);
      if (!existingType) {
        throw new Error(`Inventory type with id ${id} not found`);
  ...
  async items(@Root() type: InventoryType): Promise<InventoryItem[] | undefined> {
    try {
  ...
  async children(@Root() type: InventoryType): Promise<InventoryType[] | undefined> {
    try {
