...
describe('InventoryLocationResolver (integration)', () => {
  ...
  beforeAll(async () => {
    ...
    const testEnv: Awaited<ReturnType<typeof prepareGraphqlTest>> = await prepareGraphqlTest();
  ...
  it('should list inventory locations with filter', async () => {
    ...
    expect(res.errors).toBeUndefined();
    expect(
      (res.data.inventoryLocations as Array<{ id: string }>).some((l) => l.id === seededId)
    ).toBe(true);
  });
  ...
  it('should filter inventory locations by rootOnly', async () => {
    ...
    locations.forEach((l) => {
      expect(l.parent === null || l.parent === undefined || l.parent === '').toBe(true);
    });
