...
} from '@/backend/repositories/inventory-item';
import {
  INVENTORY_LOCATION_REPOSITORY,
  type InventoryLocationRepository,
} from '@/backend/repositories/inventory-location';
...
export class InventoryLocationResolver {
  ...
  async inventoryLocationByBarcode(
    ...
    try {
      const location = await this.inventoryLocationRepository.findByBarcode(barcode);
      return location ? this.mapToGraphQL(location) : null;
    } catch (error) {
  ...
  async updateInventoryLocation(
    ...
    try {
      const existingLocation = await this.inventoryLocationRepository.getById(id);
      if (!existingLocation) {
  ...
  @Authorized()
  @Mutation(() => Boolean)
  async deleteInventoryLocation(@Arg('id', () => String) id: string): Promise<boolean> {
    try {
      const existingLocation = await this.inventoryLocationRepository.getById(id);
      if (!existingLocation) {
        throw new Error(`Inventory location with id ${id} not found`);
  ...
  async items(@Root() location: InventoryLocation): Promise<InventoryItem[] | undefined> {
    try {
      const items = await this.inventoryItemRepository.listByQuery({
        where: { locationId: location.id },
      });

      const locationEntity = await this.inventoryLocationRepository.getById(location.id);
      if (!locationEntity) {
      ...
      return Promise.all(items.map((item) => this.inventoryResolver.mapToGraphQL(item)));
    } catch (error) {
  ...
  async itemCount(@Root() location: InventoryLocation): Promise<number | undefined> {
    try {
      // Use the countByLocationId method for better performance when only the count is needed
      const count = await this.inventoryItemRepository.countByLocationId(location.id);
      return count;
    } catch (error) {
  ...
  async children(@Root() location: InventoryLocation): Promise<InventoryLocation[] | undefined> {
    try {
  ...
  async parentName(@Root() location: InventoryLocation): Promise<string | undefined> {
    if (!location.parent) {
      return undefined;
    }
    const parent = await this.inventoryLocationRepository.getById(location.parent);
    return parent?.name;
  }
