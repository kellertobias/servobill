...
import { Expense, ExpenseInput, ExpenseWhereInput } from './expenses.schema';
...
export class ExpenseResolver {
  ...
  @Authorized()
  @Mutation(() => Boolean)
  async purgeExpenses(@Arg('confirm', () => String) confirm: string): Promise<boolean> {
    if (confirm !== 'confirm') {
  ...
  @Authorized()
  @Mutation(() => Expense)
  async createExpense(@Arg('data', () => ExpenseInput) data: ExpenseInput): Promise<Expense> {
  ...
  async category(@Root() expense: Expense): Promise<ExpenseCategoryType | null> {
    if (!expense.categoryId) {
      return null;
    }
    const settings = await this.settingsRepository.getSetting(ExpenseSettingsEntity);
    const found = settings.categories.find((cat) => cat.id === expense.categoryId);
  ...
  async attachments(@Root() expense: Expense): Promise<Attachment[] | undefined> {
