...
export class CustomerResolver {
  ...
  async customer(@Arg('id', () => String) id: string): Promise<Customer | null> {
  ...
  @Authorized()
  @Mutation(() => Customer)
  async createCustomer(@Arg('data', () => CustomerInput) data: CustomerInput): Promise<Customer> {
    const customer = await this.repository.create();

    if (!data.customerNumber) {
      const settings = await this.settingsRepository.getSetting(InvoiceSettingsEntity);
      if (!settings) {
