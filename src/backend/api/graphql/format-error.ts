...
export function formatError(error: GraphQLError): GraphQLFormattedError {
  ...
  if (error.extensions?.exception?.stacktrace?.[0]?.includes('Session Expired - Refreshable')) {
    return {
  ...
  return tracer.startActiveSpan('formatError', (span) => {
    ...
    if (exception?.validationErrors || error.extensions?.code === 'GRAPHQL_VALIDATION_ERROR') {
    ...
    if (
      ...
      const electroMessage = error.message || exception.message || 'Validation error';
      if (process.env.NODE_ENV !== 'production') {
