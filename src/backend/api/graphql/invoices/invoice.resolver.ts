...
import { INVOICE_REPOSITORY, type InvoiceRepository } from '@/backend/repositories/invoice';
...
export class InvoiceResolver {
  ...
  async createInvoice(
    ...
    const setting = await this.settingsRepository.getSetting(InvoiceSettingsEntity);
  ...
  @Authorized()
  @Mutation(() => Boolean)
  async purgeInvoices(@Arg('confirm', () => String) confirm: string): Promise<boolean> {
    if (confirm !== 'confirm') {
  ...
  async importInvoices(
    ...
    for (const invoiceData of data) {
      ...
      if (customers.has(invoiceData.customerId)) {
        ...
        customer = await this.customerRepository.getById(invoiceData.customerId);
        if (!customer) {
          throw new Error('Customer not found');
        }
      ...
      invoice.createdAt = invoiceData.invoicedAt || invoiceData.offeredAt || new Date();
  ...
  async deleteInvoiceAttachmentActivity(
    ...
    if (activity.attachmentId) {
      const attachment = await this.attachmentRepository.getById(activity.attachmentId);
