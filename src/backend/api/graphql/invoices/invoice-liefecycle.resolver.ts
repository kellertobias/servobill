...
export class InvoiceLifecycleResolver {
  ...
  @Authorized()
  @Mutation(() => InvoiceChangedResponse)
  async invoiceDeleteDraft(@Arg('id', () => String) invoiceId: string): Promise<Invoice> {
    const invoice = await this.invoiceRepository.getById(invoiceId);
    if (!invoice) {
      throw new Error('Invoice not found');
    }
      ...
      whenTime && whenTime > now
        ? await (async () => {
            ...
            if (invoice.scheduledSendJobId) {
              await this.timeBasedJobRepository.delete(invoice.scheduledSendJobId);
            }
  ...
  async invoicePdf(@Arg('id', () => String) invoiceId: string): Promise<string | null> {
    const invoice = await this.invoiceRepository.getById(invoiceId);
    ...
    if (invoice.pdf && invoice.contentHash === invoice.pdf?.forContentHash) {
      ...
      if (!dayjs(invoice.pdf.requestedAt).isBefore(dayjs().subtract(1, 'minute'))) {
