...
import { CompanyDataSetting, InvoiceSettingsEntity } from '@/backend/entities/settings.entity';
...
describe('SystemResolver Mutations', () => {
  ...
  it('should update settings', async () => {
    ...
    expect(data?.updateSettings.invoiceNumbersTemplate).toBe('INV-{year}-{counter}');
  ...
  it('should send a test event and return an event ID', async () => {
    ...
    expect(errors).toBeUndefined();
    expect(typeof data?.sendTestEvent === 'string' || data?.sendTestEvent === undefined).toBe(true);
  });
  ...
  it('should remove and add expense categories correctly', async () => {
    ...
    const categoriesWithIds = response.data.updateExpenseSettings.map((cat: any) => ({
      categoryId: cat.id,
      name: cat.name,
      color: cat.color,
    ...
    expect(response.data?.updateExpenseSettings).toHaveLength(3);
    // The GraphQL response is dynamic, so we use 'any' for mapping here (acceptable in integration tests)
    const names = response.data.updateExpenseSettings.map((cat: any) => cat.name);
  ...
  it('should re-map expense categories for imported expenses', async () => {
    ...
    const initialCategories = [{ name: 'ImportCat', color: '#abcabc', isDefault: true }];
    let response = await execute({
      source: gql`
				mutation UpdateExpenseSettings(
					$categories: [ExpenseCategoryInputType!]!
...
describe('SystemResolver', () => {
  ...
  it('should return the settings', async () => {
    // Arrange
    const companySetting = await settingsRepository.getSetting(CompanyDataSetting);
    ...
    const invoiceSettings = await settingsRepository.getSetting(InvoiceSettingsEntity);
