 ...
 */
import { extractToken, makeTokenCookie, returnUnauthorized, Session, withSession } from './session';
...
describe('withSession', () => {
  ...
  it('should return unauthorized if token expired', async () => {
    ...
    expect((res as APIGatewayProxyStructuredResultV2).body).toContain('Session Expired');
  });
  it('should return unauthorized if no token and requireActiveSession', async () => {
    ...
    expect((res as APIGatewayProxyStructuredResultV2).statusCode).toBe(401);
    expect((res as APIGatewayProxyStructuredResultV2).body).toContain('No Active Session');
  });
});
