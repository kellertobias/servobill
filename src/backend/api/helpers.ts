...
export const getSiteUrl = (evt: Parameters<APIGatewayProxyHandlerV2>[0]) => {
  ...
  const apiHost = host?.includes('localhost') ? 'localhost:3000' : `api.${siteHost}`;
...
export const getBody = (evt: Parameters<APIGatewayProxyHandlerV2>[0]) => {
  ...
  const contentType = evt.headers['Content-Type'] || evt.headers['content-type'];

  switch (contentType) {
    case 'text/plain': {
      return evt.isBase64Encoded ? Buffer.from(evt.body!, 'base64').toString() : evt.body;
    }
    case 'application/json': {
      return JSON.parse(
        evt.isBase64Encoded ? Buffer.from(evt.body!, 'base64').toString() : evt.body
      );
    }
  ...
  const bodyRaw = evt.isBase64Encoded ? Buffer.from(evt.body!, 'base64').toString() : evt.body;
