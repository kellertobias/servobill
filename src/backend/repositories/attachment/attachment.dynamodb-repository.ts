...
> {
  ...
  protected linkStore: Entity<string, string, string, typeof attachmentExpenseLinkSchema.schema>;
  ...
  constructor(
    ...
    this.linkStore = this.dynamoDb.getEntity(attachmentExpenseLinkSchema.schema);
  }
  ...
  public async createWithId(id: string, input: AttachmentCreateInput): Promise<AttachmentEntity> {
   ...
   */
  public ormToDomainEntitySafe(orm: Omit<AttachmentOrmEntity, 'storeId'>): AttachmentEntity {
   ...
   */
  public domainToOrmEntity(domain: AttachmentEntity): Omit<AttachmentOrmEntity, 'storeId'> {
    return {
   ...
   */
  public generateEmptyItem(id: string, input: AttachmentCreateInput): AttachmentEntity {
    return new AttachmentEntity({
      id,
  ...
  private async populateExpenseLinks(
    ...
    const linksToDelete = deleteAllLinks
      ? existingLinks.data
      : existingLinks.data.filter((link) => !entity.expenseIds?.includes(link.expenseId));
