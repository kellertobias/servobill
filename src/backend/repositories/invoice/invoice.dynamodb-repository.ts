...
import { entitySchema, type InvoiceOrmEntity } from './dynamodb-orm-entity';
  ...
  implements InvoiceRepository
{
   ...
   */
  public ormToDomainEntitySafe(entity: Omit<InvoiceOrmEntity, 'storeId'>): InvoiceEntity {
    return new InvoiceEntity({
      ...
      submissions: entity.submissions
        ? CustomJson.fromJson<InvoiceSubmissionEntity[]>(entity.submissions).map(
   ...
   */
  public domainToOrmEntity(domainEntity: InvoiceEntity): Omit<InvoiceOrmEntity, 'storeId'> {
    ...
    return {
      ...
      invoicedAt: domainEntity.invoicedAt ? domainEntity.invoicedAt.toISOString() : '',
      offeredAt: domainEntity.offeredAt ? domainEntity.offeredAt.toISOString() : '',
      ...
      processedEventIds: CustomJson.toJson(domainEntity.processedEventIds) || '[]',
  ...
  public async listByQuery(query: {
    ...
    let results = data.data.map((elm: InvoiceOrmEntity) => this.ormToDomainEntity(elm));
    ...
    if (query.where?.type) {
      results = results.filter((inv: InvoiceEntity) => inv.type === query.where?.type);
    }
    if (query.where?.status) {
      results = results.filter((inv: InvoiceEntity) => inv.status === query.where?.status);
    }
