  ...
  implements SessionRepository
{
   ...
   */
  protected ormToDomainEntitySafe(entity: Omit<SessionOrmEntity, 'storeId'>): SessionEntity {
   ...
   */
  public domainToOrmEntity(domainEntity: SessionEntity): Omit<SessionOrmEntity, 'storeId'> {
    return {
  ...
  public async getSession(sessionId: string): Promise<SessionEntity | null> {
    const data = await this.store.get({ sessionId, storeId: this.storeId }).go();
    if (!data || !data.data) {
      return null;
    }
  ...
  public async createSession(
    session: Omit<SessionEntity, 'sessionId' | 'createdAt' | 'updatedAt'> & Partial<SessionEntity>
  ...
  public async updateSession(
    ...
    await this.store.patch({ sessionId, storeId: this.storeId }).set(patchData).go();
  }

  /**
   * Deletes a session by sessionId.
   */
  public async deleteSession(sessionId: SessionEntity['sessionId']): Promise<void> {
