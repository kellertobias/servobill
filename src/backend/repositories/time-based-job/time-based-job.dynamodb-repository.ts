...
import { entitySchema, type TimeBasedJobOrmEntity } from './dynamodb-orm-entity';
  ...
  implements TimeBasedJobRepository
{
  ...
  public async listDueJobs(timestamp: number): Promise<TimeBasedJobEntity[]> {
    ...
    return results.data.map((item: TimeBasedJobOrmEntity) => this.ormToDomainEntitySafe(item));
  }
}
