...
import { type ExpenseOrmEntity, entitySchema } from './dynamodb-orm-entity';
  ...
  implements ExpenseRepository
{
   ...
   */
  public ormToDomainEntitySafe(entity: Omit<ExpenseOrmEntity, 'storeId'>): ExpenseEntity {
   ...
   */
  public domainToOrmEntity(domainEntity: ExpenseEntity): Omit<ExpenseOrmEntity, 'storeId'> {
    return {
  ...
  public async listByQuery(query: {
    ...
    const data = await this.store.query
      ...
      .go();
    let results = data.data.map((elm: ExpenseOrmEntity) => this.ormToDomainEntity(elm));
