...
import { entitySchema, type InventoryTypeOrmEntity } from './dynamodb-orm-entity';
import type { InventoryTypeRepository } from './interface';

type InventoryTypeSchema = typeof entitySchema.schema;
type InventoryTypeSchemaResponseItem = ResponseItem<string, string, string, InventoryTypeSchema>;
  ...
  implements InventoryTypeRepository
{
  ...
  public ormToDomainEntitySafe(
    ...
    return new InventoryTypeEntity({
      ...
      properties: entity.properties ? CustomJson.fromJson(entity.properties) : [],
  ...
  public async listByQuery(query: {
    ...
    let queryExecutor:
      | QueryBranches<
          ...
          IndexCompositeAttributes<string, string, string, InventoryTypeSchema, 'byName' | 'byId'>
        >
      | QueryOperations<
          string,
          string,
    ...
    let entities = data.data.map((elm: InventoryTypeOrmEntity) => this.ormToDomainEntity(elm));
