...
export const StatsDisplay: React.FC<{
  ...
  loading ? (
    ...
    <dl
      onClick={onClick}
      className={clsx(className, 'mx-auto grid grid-cols-1 gap-px bg-gray-900/5', {
    ...
    >
      {stats.map((stat) => {
        const changeType = stat.changeType || (stat.change || 0) >= 0 ? 'positive' : 'negative';
        return (
          <div
          ...
          >
            <dt className="text-xs sm:text-sm font-medium leading-6 text-gray-500">{stat.name}</dt>
            <dd
              className={clsx(
