...
import { Bars3Icon, UserCircleIcon, XMarkIcon } from '@heroicons/react/20/solid';
...
const MenuItems: React.FC<{
  ...
  renderWrapper = (renderChildren, key) => (
    <React.Fragment key={key}>{renderChildren({ active: false })}</React.Fragment>
  ),
}) => {
              ...
              className={clsx(
                isSecondaryMenu
                  ...
                  : {
                      ...
                      'rounded-md px-3 py-2 text-sm font-medium': isFull && !isSecondaryMenu,
                        ...
                        >
                          <Menu.Items className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                            <MenuItems
                              ...
                              renderWrapper={(renderChildren, key) => (
                                <Menu.Item key={key}>{renderChildren}</Menu.Item>
                              )}
                              as={'a'}
                              isFull
                              isSecondaryMenu
          ...
          <Disclosure.Panel className="border-b border-gray-700 md:hidden bg-gray-800/95 backdrop-blur fixed z-50 top-18 left-0 right-0 bottom-0 h-[calc(100vh-60px)] border-t border-t-gray-700">
            ...
            <div className="border-t border-gray-700 pb-3">
              <div className="mt-3 space-y-1 px-2">
                <MenuItems items={userNavigation} as={'a'} isMobile isSecondaryMenu />
              </div>
            </div>
          </Disclosure.Panel>
        </>
