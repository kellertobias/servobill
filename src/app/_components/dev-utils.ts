  ...
  try {
    ...
    for (const item of items) {
      try {
        ...
        console.log(`[deleteAllInventoryData] Deleted item: ${item.name} (${item.id})`);
      } catch (error) {
        // eslint-disable-next-line no-console
        console.error(`[deleteAllInventoryData] Error deleting item: ${item.id}`, error);
      }
    }
    ...
    for (const type of types) {
      try {
        ...
        // eslint-disable-next-line no-console
        console.log(`[deleteAllInventoryData] Deleted type: ${type.name} (${type.id})`);
      } catch (error) {
        // eslint-disable-next-line no-console
        console.error(`[deleteAllInventoryData] Error deleting type: ${type.id}`, error);
      }
    }
    ...
    for (const loc of locations) {
      try {
        ...
        // eslint-disable-next-line no-console
        console.log(`[deleteAllInventoryData] Deleted location: ${loc.name} (${loc.id})`);
      } catch (error) {
        // eslint-disable-next-line no-console
        console.error(`[deleteAllInventoryData] Error deleting location: ${loc.id}`, error);
      }
    }
    // eslint-disable-next-line no-console
...
export async function populateDemoInventory(): Promise<void> {
  try {
    ...
    types.forEach((type) => {
      type.items = items.filter((item) => item.type && item.type.id === type.id);
      type.itemCount = type.items.length;
    });
    locations.forEach((loc) => {
      loc.items = items.filter((item) => item.location && item.location.id === loc.id);
    ...
    for (const item of items) {
      const input = {
        ...
        locationId: item.location ? locationIdMap[item.location.id] : fallbackLocationId,
      ...
      try {
        ...
        // eslint-disable-next-line no-console
        console.error('[populateDemoInventory] Error creating item:', input, error);
      }
    }
    ...
    // eslint-disable-next-line no-console
    console.log('[populateDemoInventory] Demo inventory data generated and stored in backend.');
  } catch (error) {
    // eslint-disable-next-line no-console
...
export async function fetchAllInventoryData(): Promise<{
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.log('==== Inventory Locations ====', locationsRes.inventoryLocations);
    // eslint-disable-next-line no-console
...
if (typeof window !== 'undefined') {
  (window as unknown as { devUtils?: Record<string, unknown> }).devUtils =
    (window as unknown as { devUtils?: Record<string, unknown> }).devUtils || {};
  (window as unknown as { devUtils: Record<string, unknown> }).devUtils.populateDemoInventory =
    populateDemoInventory;
  (window as unknown as { devUtils: Record<string, unknown> }).devUtils.fetchAllInventoryData =
    fetchAllInventoryData;
  (window as unknown as { devUtils: Record<string, unknown> }).devUtils.deleteAllInventoryData =
