...
import { type AttachmentFilePartial, downloadAttachment } from '@/api/download-attachment';
...
export const AttachmentFileList: React.FC<AttachmentFileListProps> = ({
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      if (onDelete) {
        ...
        await query({
          query: DELETE_ATTACHMENT_MUTATION as TypedDocumentNode<unknown, unknown>,
          variables: { attachmentId: id },
        });
      }
    } catch (error_: unknown) {
      const errorMsg = error_ instanceof Error ? error_.message : 'Delete failed';
      setError(errorMsg);
    } finally {
      setDeletingId(null);
    }
  ...
  const handleDownload = async (file: AttachmentFilePartial) => {
    ...
    try {
      ...
      const errorMsg = error_ instanceof Error ? error_.message : 'Download failed';
      setError(errorMsg);
    }
  };
