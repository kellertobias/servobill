...
export function InvoicesTable({
  ...
  return (
    <Table
      ...
      title={title}
      keyField="id"
      getCategory={(data) => dayjs(data.invoicedAt || data.createdAt).format('DD.MM.YYYY')}
              ...
              >
                ...
                <div className="flex-auto">
                  <div className="flex items-start gap-x-3">
                    <div
                      className={clsx('text-sm font-medium leading-6 text-gray-900', {
                        'opacity-50': invoice.status === InvoiceStatus.Cancelled,
                      })}
                    >
                  ...
                  <div className="mt-1 text-xs leading-5 text-gray-500">
                    <span className="block sm:hidden font-semibold">{invoice.customer.name} </span>
                    {invoice.paidAt ? (
                      <span>Paid At: {dayjs(invoice.paidAt).format('DD.MM.YYYY')}</span>
                    ) : invoice.dueAt ? (
                      <span>Due At: {dayjs(invoice.dueAt).format('DD.MM.YYYY')}</span>
                    ) : null}
                    {invoice.paidCents && invoice.paidCents !== invoice.totalCents ? (
                      <span>Already Paid: {API.centsToPrice(invoice.paidCents)} €</span>
                    ) : null}
                  </div>
                </div>
              </div>
        ...
        },
        {
          ...
          render: (invoice) => (
            ...
            >
              ...
              <div className="mt-1 text-xs leading-5 text-gray-500">{invoice.subject}</div>
            </div>
          ),
        },
        {
              ...
              <div className="flex justify-end">
                ...
                >
                  ...
                  <span className="sr-only">
                    , {invoice.invoiceNumber || invoice.offerNumber}, {invoice.customer.name}
                  </span>
                </a>
              </div>
              <div className="mt-1 text-xs leading-5 text-gray-500">
