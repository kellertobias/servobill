 ...
 */
export const AttachmentDropzoneUpload: React.FC<AttachmentDropzoneUploadProps> = ({
  readOnly = false,
  ...
  const handleFiles = async (fileList: FileList | null) => {
    ...
    for (const file of newFiles) {
      ...
      try {
        ...
        const errorMsg = error_ instanceof Error ? error_.message : 'Upload failed';
        onError?.(errorMsg);
      } finally {
        setUploading(false);
      }
    ...
    >
      <div className="text-center">
        <PhotoIcon
          ...
          className={clsx('mx-auto text-gray-300', compact ? 'w-6 h-6' : 'w-12 h-12')}
        />
        <div className={clsx('mt-4 flex text-sm/6 text-gray-600', compact && 'mt-2')}>
          <label
            htmlFor="file-upload"
            className={clsx(
        ...
        <p className={clsx('text-xs/5 text-gray-600', compact && 'text-[10px]')}>
