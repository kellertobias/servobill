...
export const DateInput: React.FC<{
	...
	const currentDate =
		...
		(typeof defaultValue === 'string'
			? dayjs(defaultValue, format).toDate()
			: defaultValue) ||
		new Date();
	...
	const onHandlePickerClick = React.useMemo(() => {
		return (newValue: Date, close: () => void) => {
			...
			const isSameMonth =
				dayjs(newValueDecoupled).format('MM.YYYY') ===
				dayjs(dateRef.current).format('MM.YYYY');
					...
					>
						{reduced ? (
							<input
								...
								value={
									value
										? dayjs(selectedDate).format(displayFormat ?? format)
										: ''
								}
								onChange={(value) => {
									if (!value) {
