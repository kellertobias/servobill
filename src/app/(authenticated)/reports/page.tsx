  ...
  const stats: StatsDisplayStat[] = [
    ...
    data?.reference?.invoiceTaxCents != 0 || data?.report?.invoiceTaxCents != 0
      ? {
          ...
          change: API.getChange(data?.report?.invoiceTaxCents, data?.reference?.invoiceTaxCents),
        }
      : {
          name: 'Expenses',
          value: `${API.centsToPrice(data?.report?.expensesCents)} €`,
          change: API.getChange(data?.report?.expensesCents, data?.reference?.expensesCents),
        },
    {
      name: 'Expended Tax',
      value: `${API.centsToPrice(data?.report?.expensesTaxCents)} €`,
      change: API.getChange(data?.report?.expensesTaxCents, data?.reference?.expensesTaxCents),
    },
  ];
  return (
    <>
          ...
          columns={[
            ...
            },
            {
              ...
              render: (item) => (
                <>
                  <div className="mt-1 text-xs leading-5 text-gray-500">
                    <span className="text-gray-900 truncate">{item.description}</span>
                  </div>
                </>
              ),
            },
            {
              ...
              render: (item) => (
                <>
                  ...
                  <div className="mt-1 text-xs leading-5 text-gray-500">
                    <span className="text-gray-900">{API.centsToPrice(item.surplusCents)} €</span>
                  </div>
                  {item.taxCents ? (
                    <div className="mt-1 text-xs leading-5 text-gray-500">
                      <span className="text-gray-900">{API.centsToPrice(item.taxCents)} € Tax</span>
                    </div>
                  ) : null}
                </>
              ),
      ...
      <PageContent title="Report Builder" noCard contentClassName="pt-6">
        <PageCard>
          <SettingsBlock
            ...
            subtitle={
              <div className="prose prose-sm leading-4 text-xs text-gray-500/80">
                Select the start and end dates for the report you want to generate.
                <br />
                Quick Range select:{' '}
                <a
                  onClick={() => {
                ...
                <a
                  onClick={() => {
                    ...
                    setEndDate(undefined);
                    setTimeout(() => {
                      setStartDate(dayjs().subtract(365, 'days').format('YYYY-MM-DD'));
        ...
        {startDate?.trim() && endDate?.trim() ? (
          ...
          <PageCard>
            <div className="text-center text-sm text-gray-400">
              Once you have selected the date range, you see a preview of the report here.
            </div>
          </PageCard>
        )}
      </PageContent>
