...
export default function InventoryScanPage() {
  ...
  const [item, setItem] = useState<FindInventoryItemQuery['inventoryItem'] | null>(null);
  ...
  const handleScan = useCallback(
    async (code: string) => {
      ...
      if (
        (code === 'action: pass' || code === 'action: retest' || code === 'action: fail') && // Only submit if an item is loaded
        item
      ) {
        const stateMap = {
          'action: pass': 'PASS',
      ...
      <div className="flex-0 flex flex-col items-center justify-center bg-gray-100 border-b pt-4 pb-4">
        ...
        >
          <span className="text-sm font-medium items-center gap-2">
            Scan with <span className={clsx({ 'font-bold': useCamera })}>Camera</span> /{' '}
        ...
        <div className="flex flex-col py-2">
          ...
          {loading ? (
            ...
            <div className="w-full max-w-2xl p-4 ">
              ...
              <div className="mb-1">
                <b>Type:</b> {item.type?.name || <span className="text-gray-400">None</span>}
              </div>
              <div className="mb-1">
                <b>Location:</b>{' '}
                {item.location?.name || <span className="text-gray-400">None</span>}
              </div>
              <div className="mb-1">
            ...
            <div className="text-gray-400">Scan a barcode or QR code to find an item.</div>
          )}
        </div>
