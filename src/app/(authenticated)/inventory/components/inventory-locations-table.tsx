...
import type { InventoryLocationsListQuery } from '@/common/gql/graphql';
...
type InventoryLocation = NonNullable<InventoryLocationCategory['children']>[number];
  ...
  return (
    <Table<InventoryLocationFlattened>
      ...
      renderCategoryDivider={(location) => (
        <div
          onClick={(e) => {
            if (e.target instanceof HTMLButtonElement || e.currentTarget.closest('button')) {
              return;
            }
        ...
        >
          ...
          <div className="flex justify-end gap-2">
            <Button small secondary onClick={() => openEditDrawer?.(location.parent.id)}>
              Edit
            </Button>
          </div>
        </div>
      )}
      getLineLink={onRowClick ? (location) => () => onRowClick(location.id) : undefined}
      columns={[
        {
          key: 'name',
          title: 'Location Name',
          ...
          render: (location) => (
            <>
              <div className="text-sm font-medium leading-6 text-gray-900">
                ...
                {location.isEmpty ? (
                  <span className="italic text-gray-400">No locations in this category</span>
                ) : (
                  location.name
                )}
              </div>
              {!location.isCategory && !location.isEmpty && (
                <div className="mt-1 text-xs leading-5 text-gray-500">
                  <span className="text-gray-900">{location.itemCount} items</span>
                </div>
              )}
            </>
          ),
