...
const useTypeProperties = (
  ...
  const [defaultPropertyKeys, setDefaultPropertyKeys] = React.useState<string[]>([]);
   ...
   */
  const fetchTypeProperties = React.useCallback(async (typeId: string | null | undefined) => {
    if (!typeId) {
...
const useInventoryItemDrawer = ({
  ...
  const { defaultPropertyKeys } = useTypeProperties(data.type, (nextProperties) => {
    setData((current) => ({
      ...current,
...
const EditInventoryItemDrawer = forwardRef(
  (
    ...
    const { drawerId, data, setData, defaultPropertyKeys, onSave, onCreatedRef, handleClose } =
    ...
    const [creating, setCreating] = React.useState<'next' | 'open' | null>(null);
      ...
      >
        <div className="divide-y divide-gray-200 px-4 sm:px-6">
          <div className="space-y-6 pb-5 pt-6">
            ...
            <InventoryTypeSelect
              value={data.type}
              onChange={(type) => setData((current) => ({ ...current, type: type ?? '' }))}
            />
            {/* Location select */}
                ...
                {data.properties?.map((prop, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    ...
                    {isDefaultPropertyKey(prop.key) ? (
                      <span className="w-32 text-sm text-gray-700">{prop.key}</span>
                    ) : (
                      <input
                        type="text"
                        value={prop.key}
                    ...
                    <Input
                      value={prop.value}
                      onChange={(value) => handlePropertyChange(idx, 'value', value)}
