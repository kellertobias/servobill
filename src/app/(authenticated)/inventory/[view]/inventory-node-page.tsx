...
import type { InventoryItem, InventoryLocation, InventoryType, InventoryView } from '../types';
 ...
 */
function useNodeInventoryItems(id: string | undefined, view: InventoryView, searchQuery?: string) {
 ...
 */
function InventoryItemsList({ items, loading }: { items: InventoryItem[]; loading: boolean }) {
        ...
        <ul className="divide-y divide-gray-200 cursor-pointer">
          {items.map((item) => (
            ...
            >
              ...
              <span className="text-xs text-gray-500">Type: {item.type?.name || '—'}</span>
              <span className="text-xs text-gray-500">Location: {item.location?.name || '—'}</span>
              <span className="text-xs text-gray-500">Last scanned: {item.lastScanned}</span>
            </li>
          ))}
        </ul>
      )}
...
export default function InventoryNodePage({
  ...
  const { entries, node, loading, searchQuery, setSearchQuery, reload } = useInventoryListData({
    view: params.view,
    id: params.id,
  });
  ...
  const { items, loading: itemsLoading } = useNodeInventoryItems(params.id, view, searchQuery);
  ...
  const openNewItemDrawer = () => {
    ...
    const parentLocationId = view === 'location' ? (node?.id ?? undefined) : undefined;
  ...
  const NodeInfoCard = node ? (
    <PageCard className="mb-4">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
        <div>
          <span className="font-semibold">Inventory {view === 'type' ? 'Type' : 'Location'}</span>
  ...
  return (
    <>
      {view === 'type' ? (
        ...
        <EditInventoryLocationDrawer ref={locationDrawerRef} onReload={reload} />
      )}
        ...
        right={
          <>
            <div className="flex flex-row gap-2 mb-4">
              ...
              <Button icon={PlusIcon} onClick={openNewItemDrawer} className="min-w-[130px]" header>
                Add Item
              </Button>
              <Button
                icon={PlusIcon}
      ...
      >
        <InventoryHeader
          ...
          onTabChange={(tab) => router.push(`/inventory/${tab === 'types' ? 'type' : 'location'}`)}
