 ...
 */
export const InventoryActivityForm: React.FC<InventoryActivityFormProps> = ({ itemId, reload }) => {
  ...
  const ref = useAutoSizeTextArea(
    note || (checkState ? 'Optional note for check result...' : 'Add a note...')
  );
  ...
  return (
    <div className={clsx('mt-6 flex gap-x-3 w-full flex-row -mr-px')}>
      <form className={clsx('relative flex-grow min-w-0')} onSubmit={handleSubmit}>
        <div
          className={clsx(
        ...
        >
          <div className={clsx('flex p-[2px]')}>
            <textarea
              ...
              rows={2}
              placeholder={checkState ? 'Optional note for check result...' : 'Add a note...'}
              value={note}
          ...
          >
            <div className={clsx('flex-1 max-w-xs')}>
              <SelectInput
                clearable={false}
                value={checkState || ''}
                onChange={(val) => setCheckState((val ?? '') as InventoryCheckState | '')}
