 ...
 */
export const InventoryItemProperties: React.FC<InventoryItemPropertiesProps> = ({
  properties,
  setProperties,
  setDetailsChanged,
}) => {
  ...
  return (
    <div className="pt-6">
      ...
      <InvoiceItemDivider onAddItem={() => handleAddProperty(0)} locked={false} />
      {/* Properties list */}
          ...
          <React.Fragment key={idx}>
            <div className="relative">
              <div className="flex gap-2 items-center py-2 text-sm">
                ...
                <InvoiceRowDeleteButton onClick={() => handleRemoveProperty(idx)} />
              </div>
            </div>
            {/* Divider/add button between each row (except after last) */}
            {idx < properties.length - 1 && (
              <InvoiceItemDivider onAddItem={() => handleAddProperty(idx + 1)} locked={false} />
            )}
          </React.Fragment>
        ))
      ) : (
      ...
      <InvoiceItemDivider onAddItem={() => handleAddProperty(properties.length)} locked={false} />
    </div>
  );
};
