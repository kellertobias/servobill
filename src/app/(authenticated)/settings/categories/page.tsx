...
export default function CategoriesSettingsPage() {
  ...
  const handleAddCategory = () => {
    ...
    setData((current) => [
      ...((current ?? []).filter(Boolean) as ExpenseCategoryType[]).map((cat) => ({
        ...cat,
        color: cat.color ?? '#cccccc',
        isDefault: cat.isDefault ?? false,
        sumForTaxSoftware: cat.sumForTaxSoftware ?? false,
  ...
  const handleUpdateCategory = (id: string, updated: Partial<ExpenseCategoryType>) => {
    setData((current) =>
      ((current ?? []) as ExpenseCategoryType[]).map((cat) =>
        cat.id === id
          ? {
              ...
              sumForTaxSoftware: updated.sumForTaxSoftware ?? cat.sumForTaxSoftware ?? false,
  ...
  return (
    <PageContent title="Settings - Categories" noPadding contentClassName="overflow-hidden pt-6">
        ...
        <SettingsBlock
          ...
          subtitle={
            <div className="prose prose-sm leading-4 text-xs text-gray-500/80">
              Here you can add, edit, or remove categories for expenses and products. Categories
              help you organize and report your data.
            </div>
          }
        >
          {/* List of categories */}
          <div className="space-y-8">
            {Array.isArray(data) && data.length > 0 ? (
              data.map((cat: ExpenseCategoryType) => (
                <div key={cat.id} className="border-b pb-6 last:border-b-0 last:pb-0 space-y-2">
                  ...
                  <div className="flex w-full gap-2">
                    <div className="w-10/12">
                      <Input
                        label="Name"
                        value={cat.name}
                        onChange={(name) => handleUpdateCategory(cat.id, { name })}
                        className="w-full"
                      />
                    </div>
                    <div className="w-2/12 flex items-end">
                      <Input
                        ...
                        type="color"
                        onChange={(color) => handleUpdateCategory(cat.id, { color })}
                        className="w-full"
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex w-full gap-2">
                    ...
                    <div className="w-4/12">
                      <Input
                        label="Reference"
                        value={cat.reference ?? ''}
                        onChange={(reference) => handleUpdateCategory(cat.id, { reference })}
                        className="w-full"
                      />
                    </div>
                  </div>
                  ...
                  <div className="w-full">
                    ...
                    <Input
                      ...
                      onChange={(description) => handleUpdateCategory(cat.id, { description })}
                      as="textarea"
                      textarea
                      className="w-full"
                    />
          ...
          <div className="mt-12 border-t pt-6 space-y-2">
            ...
            <div className="flex w-full gap-2">
              ...
              <div className="w-4/12">
                <SelectInput
                  ...
                  onChange={(val) => setNewCategory((c) => ({ ...c, isDefault: val === 'yes' }))}
                  options={[
                    { value: 'yes', label: 'Yes' },
                    { value: 'no', label: 'No' },
                  ]}
              ...
              <div className="w-4/12">
                <Input
                  ...
                  onChange={(reference) => setNewCategory((c) => ({ ...c, reference }))}
                  className="w-full"
                />
              </div>
            </div>
            ...
            <div className="w-full">
              ...
              <Input
                ...
                onChange={(description) => setNewCategory((c) => ({ ...c, description }))}
                as="textarea"
                textarea
                className="w-full"
              />
