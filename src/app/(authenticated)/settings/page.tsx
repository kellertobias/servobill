...
function NumberValidity({ template, number }: { template?: string; number?: string }) {
  ...
  return nextNumberValidity === null ? (
    <div className="text-xs text-red-500/80 ">The next number is not valid for the template</div>
  ) : (
...
export default function SettingsHomePage() {
  const { data, setData, initialData, loading, reload } = useLoadData(async () => {
  ...
  if (loading || !data) {
    return (
      <PageContent title="Settings" noPadding contentClassName="overflow-hidden pt-6">
        {loading ? (
          <LoadingSkeleton />
        ) : (
          <NotFound title="No Settings" subtitle="There are no results to show" />
        )}
      </PageContent>
    );
  }
      ...
      >
        <div className="space-y-12 p-6 pt-0 divide-y divide-gray-900/10">
          <SettingsBlock
            ...
            subtitle={
              <div className="prose prose-sm leading-4 text-xs text-gray-500/80">
                These company details can be used in the invoice and email templates.
              </div>
            }
          >
            <Input
              ...
              options={[
                {
                  ...
                  label: 'VAT Enabled',
                  description: 'Invoices include VAT. Standard for most businesses.',
                },
                {
                  value: VatStatus.VatDisabledKleinunternehmer,
                  label: 'VAT Disabled (Kleinunternehmerregelung § 19 UStG)',
                  description: 'Small business exemption (Germany). No VAT on invoices.',
                },
                {
                  value: VatStatus.VatDisabledOther,
                  label: 'VAT Disabled (Other)',
                  description: 'No VAT for other legal reasons (e.g., non-profit, foreign entity).',
                },
              ]}
              className="w-full"
            />
          ...
          <SettingsBlock
            ...
            subtitle={
              <div className="prose prose-sm leading-4 text-xs text-gray-500/80">
                Define how your invoice numbers increase. You need to set a Template which is used
                to generate the invoice number. The template can contain the following placeholders:
                <ul>
                  <li>
                ...
                You can also define a separate template for the incrementing number. This is useful
                if you e.g. increasing numbers per year, but want to reset the number every year,
                while having the month and day in the invoice number.
              </div>
            }
          >
            <Input
            ...
            <p className="text-xs text-gray-500/80">
              ...
              <code>
                {Numbering.makeNumber(data?.invoiceNumbersTemplate || 'INV-YYMM-####', 42)}
              </code>
              .
            </p>
            <Input
            ...
            />
            <p className="text-xs text-gray-500/80 ">
              We search for the highest invoice number matching the template and increment it by
              one. If there is no invoice number yet, we start with 1.
            </p>
            <Input
          ...
          <SettingsBlock
            ...
            subtitle={
              <div className="prose prose-sm leading-4 text-xs text-gray-500/80">
                Define how your offer numbers increase. Works the same as invoice numbers, but has a
                separate template and next number.
              </div>
            }
          >
            <Input
            ...
            <p className="text-xs text-gray-500/80 ">
              An example number for the template above would be{' '}
              <code>{Numbering.makeNumber(data?.offerNumbersTemplate || 'OFF-YYMM-####', 21)}</code>
              .
            </p>
            <Input
              label="Increment Template"
          ...
          <SettingsBlock
            ...
            subtitle={
              <div className="prose prose-sm leading-4 text-xs text-gray-500/80">
                Define how your customer numbers increase. Works the same as invoice numbers, but
                has a separate template and next number.
              </div>
            }
          >
            <Input
            ...
            <p className="text-xs text-gray-500/80 ">
              An example number for the template above would be{' '}
              <code>
                {Numbering.makeNumber(data?.customerNumbersTemplate || 'CUS-YYMM-####', 42)}
              </code>
              .
            </p>
            <Input
          ...
          <SettingsBlock
            ...
            subtitle={
              <div className="prose prose-sm leading-4 text-xs text-gray-500/80">
                Define the E-Mail address we will send invoices, offers and reminders from. You can
                also define a reply to address.
              </div>
            }
          >
            <Input
          ...
          <SettingsBlock
            ...
            subtitle={
              <div className="prose prose-sm leading-4 text-xs text-gray-500/80">
                Here you can define the templates for the E-Mails we send. Placeholders will be
                replaced using handlebars. You can use the following placeholders in the subject and
                body:
                <ul>
                  <li>
                ...
                <ul>
                  <li>
                    <code>{'{{#if type}}...{{/if}}'}</code> This block will be shown for the type of
                    email. You can use the following types: <code>invoice</code>, <code>offer</code>
          ...
          <SettingsBlock
            ...
            subtitle={
              <div className="prose prose-sm leading-4 text-xs text-gray-500/80">
                Manage the categories used for expenses and products. You can add, edit, or remove
                categories here.
              </div>
            }
          >
                    ...
                    variables: {
                      settings: {
                        company: {
                          ...
                          web: data.companyWeb || '',
                          bankAccountHolder: data.companyBankAccountHolder || '',
                        ...
                        defaultInvoiceDueDays: Number(data.defaultInvoiceDueDays || '0'),
                        offerValidityDays: Number(data.offerValidityDays || '0'),
                        invoiceNumbersTemplate: data.invoiceNumbersTemplate,
                        invoiceNumbersIncrementTemplate: data.invoiceNumbersIncrementTemplate,
                        invoiceNumbersLast: data.invoiceNumbersLast,
                        offerNumbersTemplate: data.offerNumbersTemplate,
                        offerNumbersIncrementTemplate: data.offerNumbersIncrementTemplate,
                        offerNumbersLast: data.offerNumbersLast,
                        customerNumbersTemplate: data.customerNumbersTemplate,
                        customerNumbersIncrementTemplate: data.customerNumbersIncrementTemplate,
