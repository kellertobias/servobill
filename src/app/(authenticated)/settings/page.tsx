...
import { BackupFrequency, VatStatus } from '@/common/gql/graphql';
	...
	const { data, setData, initialData, loading, reload } = useLoadData(
		async () => {
			...
			return {
				...
				backupEmail: res.settings.backup?.backupEmail || '',
				backupFrequency:
					res.settings.backup?.backupFrequency || BackupFrequency.Daily,
				backupEnabled: res.settings.backup?.backupEnabled || false,
			};
		},
	);
		...
		>
			<div className="space-y-12 p-6 pt-0 divide-y divide-gray-900/10">
				...
				<SettingsBlock
					...
					subtitle={
						<div className="prose prose-sm leading-4 text-xs text-gray-500/80">
							Define how your offer numbers increase. Works the same as invoice
							numbers, but has a separate template and next number.
						</div>
					}
				>
					<Input
						...
						<div className="prose prose-sm leading-4 text-xs text-gray-500/80">
							...
							<ul>
								<li>
									...
									shown for the type of email. You can use the following types:{' '}
									<code>invoice</code>, <code>offer</code>,{' '}
				...
				<SettingsBlock
					...
					subtitle={
						<div className="prose prose-sm leading-4 text-xs text-gray-500/80">
							Configure automated backups of your data.
						</div>
					}
				>
					<div className="flex items-center justify-between mb-4">
						...
						<Toggle
							value={data?.backupEnabled || false}
				...
				<SettingsBlock
					...
					subtitle={
						<div className="prose prose-sm leading-4 text-xs text-gray-500/80">
							Manage the categories used for expenses and products. You can add,
							edit, or remove categories here.
						</div>
					}
				>
					<div>
					...
					onClick={() => {
						doToast({
							promise: (async () => {
								await API.query({
									query: gql(`
											mutation UpdateSettings($settings: SettingsInput!, $backupSettings: BackupSettingsInput!) {
									...
									variables: {
										backupSettings: {
											...
											backupFrequency:
												data.backupFrequency || BackupFrequency.Daily,
										},
										settings: {
											company: {
