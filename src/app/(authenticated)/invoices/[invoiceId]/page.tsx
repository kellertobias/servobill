...
export default function InvoiceDetailPage() {
  ...
  if (loading) {
    return (
      <PageContent title={`Invoice: Loading...`} noPadding contentClassName="overflow-hidden pt-6">
        <LoadingSkeleton />
      </PageContent>
    );
  }
                  ...
                  (await confirmDialog({
                    ...
                    content: (
                      <>
                        ...
                        and does not yet delete the PDF files. Also, Invoice Numbers won't be reset.
                      </>
                    ),
                  })) &&
                  ...
                  (await confirmDialog({
                    ...
                    content: (
                      <>
                        ...
                        cannot be undone and might not be legal according to the tax law in your
                        country.
                      </>
                    ),
                  }))
        ...
        <div className="mx-auto grid max-w-2xl grid-cols-1 grid-rows-1 items-start gap-x-8 gap-y-8 lg:mx-0 lg:max-w-none lg:grid-cols-3">
          ...
          <div className="lg:col-start-3 lg:row-end-1">
            ...
            {(data.status === InvoiceStatus.Paid ||
              ...
              <InvoiceStatusPaid data={data} reload={reload} hasChanges={hasChanges} />
            )}
            {data.type === InvoiceType.Invoice && data.status === InvoiceStatus.Sent && (
              <InvoiceStatusSent data={data} reload={reload} hasChanges={hasChanges} />
            )}
            {data.type === InvoiceType.Invoice && data.status === InvoiceStatus.Draft && (
              <InvoiceStatusDraft data={data} reload={reload} hasChanges={hasChanges} />
            )}
            {data.type === InvoiceType.Offer &&
              (data.status === InvoiceStatus.Sent || data.status === InvoiceStatus.Cancelled) && (
                <InvoiceStatusOfferSent data={data} reload={reload} hasChanges={hasChanges} />
              )}
            {data.type === InvoiceType.Offer && data.status === InvoiceStatus.Draft && (
              <InvoiceStatusOfferDraft data={data} reload={reload} hasChanges={hasChanges} />
            )}
            {data.status === InvoiceStatus.Cancelled && (
              <InvoiceStatusCancelled data={data} reload={reload} hasChanges={hasChanges} />
            )}
          </div>
          ...
          <div className="-mx-4  sm:mx-0  lg:col-span-2 lg:row-span-2 lg:row-end-2">
            ...
            >
              ...
              <InvoiceHeader
                data={data}
                onChange={(changes) => setData((current) => ({ ...current, ...changes }))}
                locked={locked}
              />
              <InvoicePositions
                data={data}
