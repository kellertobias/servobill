...
export type InvoiceData = NonNullable<ReturnType<typeof useInvoiceData>['data']>;
...
export const useInvoiceActivity = () => {
  ...
  const toggleAttachToEmail = React.useCallback(
    async ({
      ...
      await doToast({
        ...
        loading: attachToEmail ? 'Enabling email attachment...' : 'Disabling email attachment...',
  ...
  const deleteAttachmentActivity = React.useCallback(
    async ({ invoiceId, activityId }: { invoiceId: string; activityId: string }) => {
      if (
        !(await confirmDialog({
          danger: true,
