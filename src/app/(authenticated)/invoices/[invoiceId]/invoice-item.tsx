      ...
      <div className="relative">
        {locked ? null : (
          <InvoiceRowDeleteButton
            onClick={() => {
              onChange({
                items: [...(data || []).slice(0, index), ...(data || []).slice(index + 1)],
              });
            }}
          />
        )}
        {locked ? null : <InvoiceLookupStartButton onClick={() => showProductCatalog()} />}
        <div className={clsx(columns, 'grid py-3')} key={item.id}>
          <div className={clsx(singleColumn, 'mb-4 sm:mb-0')}>
            ...
            {item.linkedExpenses && item.linkedExpenses.length > 0 && (
              <div className="mt-2">
                <div className="font-semibold text-xs text-gray-700 mb-1">Linked Expenses</div>
                {item.linkedExpenses.map((exp, expIdx) => (
                  <label key={expIdx} className="flex items-center gap-2 text-xs">
                    <input
                      type="checkbox"
                      checked={exp.enabled}
                      disabled={locked}
                      onChange={() => {
                        const newLinkedExpenses = item.linkedExpenses!.map((e, i) =>
