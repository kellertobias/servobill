  ...
  return (
    <InlineEditableArea textRight={textRight} locked={locked}>
      {(editing, setEditing) => (
        ...
        >
          {prefix && <span className="whitespace-nowrap break-keep pr-1">{prefix}</span>}
          {editing ? (
            <textarea
              ref={ref}
              autoFocus
            ...
            <span className="whitespace-pre-line">
              {value?.trim() ? value : <span className="opacity-30">{empty || placeholder}</span>}
            </span>
          )}
          {postfix && <span className="whitespace-nowrap break-keep pl-1">{postfix}</span>}
        </div>
      )}
    </InlineEditableArea>
  );
...
export function InlineEditableArea({
  ...
  textRight?: boolean;
  onStartEditing?: () => void;
  children: (editing: boolean, setEditing: (editing: boolean) => void) => React.ReactNode;
}) {
  ...
  return (
    <div
      ...
      onClick={() => !locked && !editing && (onStartEditing ? onStartEditing() : setEditing(true))}
    >
      {!locked && (
        <button
        ...
        >
          {editing ? <CheckIcon className="w-3 h-3" /> : <PencilIcon className="w-3 h-3" />}
        </button>
      )}
