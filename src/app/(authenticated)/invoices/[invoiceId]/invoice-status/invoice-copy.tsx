...
export const onClickInvoiceCopy = async (props: {
	...
	if (
		await confirmDialog({
			primary: true,
			title: props.isOffer
				? 'Copy Offer or generate Invoice?'
				: 'Copy Invoice?',
			content: (
				<>
					<p className="mb-3">
						This will generate a new invoice or offer with the same contents as
						this {props.isOffer ? 'offer' : 'invoice'}.
						{props.isOffer ? (
							<>
								This will not affect the original offer except for adding a link
								to the new invoice.
							</>
						) : (
