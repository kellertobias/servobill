    ...
    <div className="rounded-lg bg-gray-50 shadow-sm ring-1 ring-gray-900/5">
      <dl className="flex flex-wrap">
        <div className="flex-auto pl-6 pt-6">
          <dt className="text-sm font-semibold leading-6 text-gray-900">Amount Paid</dt>
          <dd className="mt-1 text-base font-semibold leading-6 text-gray-900">
            {API.centsToPrice(data.paidCents)} € /{' '}
            <span className="font-normal text-sm">{API.centsToPrice(data.totalCents)} €</span>
          </dd>
        </div>
        ...
        <div className="mt-4 flex w-full flex-none gap-x-4 px-6">
          ...
          <dd className="text-sm leading-6 text-gray-500">
            {data.paidCents === data.totalCents ? (
              ...
              <>
                Due on{' '}
                {dayjs(data.dueAt || dayjs(data.invoicedAt).add(14, 'days')).format('DD.MM.YYYY')}
              </>
            )}
          </dd>
        </div>
        <div className="mt-4 mb-4 flex w-full flex-none gap-x-4 px-6">
          ...
          <dd className="text-sm leading-6 text-gray-500">Paid with {data.paidVia}</dd>
        </div>
        <InvoiceActions
