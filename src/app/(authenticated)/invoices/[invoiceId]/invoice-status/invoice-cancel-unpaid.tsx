...
export const onClickCancelInvoice = async (props: {
  ...
  if (hasLinkedExpenses && !props.isOffer) {
    ...
    cancelExpenses = (await confirmDialog({
      ...
      content: (
        <>
          This invoice has linked expenses. Do you also want to cancel (delete) the expenses that
          were created from this invoice?
        </>
      ),
    })) as boolean;
  }
  ...
  if (
    ...
    doToast({
      ...
      error: props.isOffer ? 'Failed to cancel offer.' : 'Failed to cancel invoice.',
    });
  }
};
