...
import { getInvoiceSubTotal, getInvoiceTaxTotal, getInvoiceTotal } from '@/common/invoice';
...
export function InvoicePositions({
  ...
  return (
    <div className="mt-16 w-full leading-6 text-sm text-gray-900">
      <div className={clsx(columns, 'grid font-semibold py-3 grid-cols-[5fr_2.4fr]')}>
        <div className={clsx(singleColumn)}>Item</div>
        <div className={clsx(singleColumn, 'text-right hidden sm:block')}>Amount</div>
        <div className={clsx(singleColumn, 'text-right hidden sm:block')}>Price</div>
        {vatStatus === 'VAT_ENABLED' && (
          <div className={clsx(singleColumn, 'text-right hidden sm:block')}>Tax</div>
        )}
      ...
      <div className="grid sm:grid-cols-2 pt-5 ">
        ...
        <div>
          ...
          <div className={clsx(itemPricePart, 'font-semibold py-1')}>
            ...
            <div className="col-span-1 text-right">{API.centsToPrice(getInvoiceTotal(data))} €</div>
          </div>
        </div>
      </div>
    </div>
