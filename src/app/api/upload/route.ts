...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!attachmentId) {
      return new Response(JSON.stringify({ error: 'Missing attachmentId parameter' }), {
        status: 400,
      });
    }
    ...
    if (contentType.startsWith('multipart/form-data')) {
      ...
      if (!boundary) {
        return new Response(JSON.stringify({ error: 'No boundary in Content-Type' }), {
          status: 400,
        });
      }
      ...
      if (parts.length !== 1 || !parts[0].filename) {
        return new Response(JSON.stringify({ error: 'Only one file per upload is allowed.' }), {
          status: 400,
        });
      }
      ...
      const part = parts[0];
      if (!part.data) {
        return new Response(JSON.stringify({ error: 'No file data found in upload.' }), {
          status: 400,
        });
      }
...
export async function POST() {
  return new Response(JSON.stringify({ error: 'Method not allowed', details: 'Use PUT instead' }), {
    status: 405,
  });
}
