...
export const downloadFile = (options: { filename: string; content: string; type?: string }) => {
  const dataUri = `data:${
...
export async function parseCsvFileOrString(
  ...
  try {
    return await new Promise((resolve, reject) => {
      if (typeof fileOrString === 'string') {
        ...
        Papa.parse(fileOrString, {
          ...
          skipEmptyLines: true,
          transformHeader: (header) => header.trim().toLowerCase(),
          complete: (results: { data: Record<string, string>[]; errors: unknown[] }) => {
    ...
    let csv = '';
    csv = typeof fileOrString === 'string' ? fileOrString : await fileOrString.text();
