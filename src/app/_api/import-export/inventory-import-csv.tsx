...
function getRowLookup(
  ...
  const lookupId = row[`${lookup}id`] ? String(row[`${lookup}id`]).trim() : null;
  if (lookupId) {
    return lookupId;
  }

  const lookupNameRaw = row[`${lookup} name`] || row[`${lookup}name`] || row[`${lookup}`];
  const lookupName = lookupNameRaw ? String(lookupNameRaw).trim().toLowerCase() : null;

  if (!lookupName) {
    return null;
  }
  return lookupData.find((l) => l.name.trim().toLowerCase() === lookupName)?.id || null;
}
