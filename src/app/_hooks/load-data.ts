...
export const useLoadData = <T, X extends Record<string, unknown> = Record<string, never>>(
  ...
  const loadData = useCallback(async () => {
    try {
      setIsLoading(true);
      setHasError(null);
      const data = await loader({ ...params, ...externalParameters } as Params & X);
  ...
  return {
    ...
    loading: isLoading,
    data: currentData,
    setData: setCurrentData as React.Dispatch<React.SetStateAction<Partial<T> | null>>,
    reload: loadData,
  };
};
...
export const useSaveCallback = <T>({
  ...
  const hasChanges = hasChangesProp || useHasChanges(initialData as never, data as never);
